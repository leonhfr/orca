...
func TestQuiescence(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			depth:  4,
			result: searchResult{nodes: 916, score: 49},
		...
		},
		{
			...
			fen:    "5r1k/4Qpq1/4p3/1p1p2P1/2p2P2/1p2P3/3P4/BK6 b - - 0 1",
			depth:  5,
			result: searchResult{nodes: 4224, score: 2},
		...
		},
		{
			...
			fen:    "5r1k/4Qpq1/4p3/1p1p2P1/2p2P2/1p2P3/3P4/BK6 b - - 0 1",
			depth:  6,
			result: searchResult{nodes: 9627, score: 2},
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			assert.Equal(t, tt.result.nodes, output.nodes)
			assert.Equal(t, tt.result.score, output.score)
